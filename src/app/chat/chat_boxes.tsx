...
import {currentUser, currentMotion, currentCommittee, pb} from "@/app/pocketbase";
...
const ChatBox: React.FC = () => {
  ...
  const [messages, setMessages] = useState<{ text: string; timestamp: string, owner: string }[]>([]);
  ...
  const sendMessage = (message: string) => {
    if (message.trim()) {
      ...
      const currentTime = getCurrentTime() + " " + new Date().toLocaleDateString();
      setMessages([...messages, { text: message, timestamp: currentTime, owner: currentUser.id }]);
  ...
  async function helper() {
    ...
    retval.expand.messages.forEach((message) => {
      ...
      helperArray.push({ text: message.text, timestamp: formattedDate, owner: message.owner});
    });
          ...
          messages.map((message, index) => (
            ...
            <Box key={index} className="mb-4 flex flex-col items-end">
              ...
              <Box className="flex items-center space-x-2">
                {message.owner === currentUser.id ? (
                  <Box className={'rounded bg-blue-500 text-white p-2 max-w-md'}>
                    {message.text}
                  </Box>
                ) : (
                  <Box className={'rounded bg-gray-300 text-white p-2 max-w-md'}>
                    {message.text}
                  </Box>
                )}
